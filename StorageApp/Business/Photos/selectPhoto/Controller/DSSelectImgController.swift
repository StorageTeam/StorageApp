...
class DSSelectImgController: DSBaseViewController {
    ...
    var itemSize : CGSize {
        get {
            let itemMargin = floor((DSConst.UISCREENWIDTH - 40) / 3)
...
//        let itemMargin = floor((UISCREENWIDTH - 40) / 3)
        ...
        collectionView.registerClass(DSFetchImgCell.self, forCellWithReuseIdentifier: String(DSFetchImgCell))
...
extension DSSelectImgController: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCellWithReuseIdentifier(String(DSFetchImgCell), forIndexPath: indexPath)
        let selected = self.isSelectedAt(indexPath)
        
        if let fetchImgCell = cell as? DSFetchImgCell {
    ...
    func collectionView(collectionView: UICollectionView, didSelectItemAtIndexPath indexPath: NSIndexPath) {
        guard let imgCell = collectionView.cellForItemAtIndexPath(indexPath) as? DSFetchImgCell else {
            return
        }
