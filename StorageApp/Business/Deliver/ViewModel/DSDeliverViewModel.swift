...
class DSDeliverViewModel: NSObject {
    ...
    func numberOfRowsInSection(section: Int) -> Int {
        if section >= 0 && section < self.dataSource.count {
            let model = self.dataSource[section] as? DSDeliverMissionModel
    ...
    func cellTypeAtIndexPath(indexPath : NSIndexPath) -> DSDeliverCell {
    ...
    func missionModelAtIndexPath(indexPath :NSIndexPath) -> DSDeliverMissionModel? {
        let idx = indexPath.section
        if idx >= 0 && idx < self.dataSource.count {
            return (self.dataSource[idx] as? DSDeliverMissionModel)
        }
        return nil
    }
