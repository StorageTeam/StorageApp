...
class DSMainViewController: DSBaseViewController, CurrentShopDelegate, DSDataCenterDelegate {
    ...
    func clickSelectImgButton(sender: UIButton) {
        ...
        let selectProductPicCon = DSSelectImgController.init(naviItemTitle: "第1步:请选择商品照片", selectDone: { (prodAssets:[PHAsset]) in
            let selectPricePicCon = DSSelectImgController.init(naviItemTitle: "第2步:请选择价签照片", selectDone: { (priceAssets:[PHAsset]) in
                ...
                editController.addImagAsset(prodAssets, isProduct: true)
                editController.addImagAsset(priceAssets, isProduct: false)
                editController.hidesBottomBarWhenPushed = true
                
                wkSelf?.replaceLastController(editController)
            })
            selectPricePicCon.maxImgCount = 30
            wkSelf?.replaceLastController(selectPricePicCon)
        })
        selectProductPicCon.maxImgCount = 30
    ...
    private func configRightNaviBarItem() -> Void {
        ...
        let switchContainer = UIView.init(frame: CGRectMake(0, 0, 32, CGRectGetHeight((self.navigationController?.navigationBar.frame)!)))
    ...
    private func configBarTitleView() -> Void {
        if DSAccount.getRoleType().isDeliverAble() {
            if (DSConst.IS_Screen_320()) {
                self.segmentControl.frame = CGRectMake(0, 0, 130, 26)
            } else {
    ...
    override func loadView() {
        ...
        var borderWidth : CGFloat = 286
        if (DSConst.IS_Screen_320() || DSConst.IS_Screen_375()) {
            borderWidth = 234
        }
        ...
        self.tipLabel.snp_makeConstraints { (make) in
            make.top.equalTo(self.curShopBgView.snp_bottom).offset(16)
            make.centerX.equalTo(self.view)
        }
        
        self.view.addSubview(self.scanCollectButton)
        self.scanCollectButton.snp_makeConstraints { (make) in
            make.top.equalTo(self.tipLabel.snp_bottom).offset(16)
    ...
    lazy var segmentControl: UISegmentedControl = {
        ...
        control.layer.borderWidth  = DSConst.pixelHeight
    ...
    lazy var statusLabel: UILabel = {
        ...
        label.text = "不接单"
        label.frame = CGRectMake(0, 0, 40, CGRectGetHeight((self.navigationController?.navigationBar.frame)!))
    ...
    lazy var statusSwitch: UISwitch = {
        ...
        var scale = 0.7
        switchBtn.transform = CGAffineTransformMakeScale(CGFloat.init(scale), CGFloat.init(scale))
        switchBtn.addTarget(self, action: #selector(self.clickSwitchAction), forControlEvents: .ValueChanged)
//        switchBtn.thumbTintColor = UIColor.init(rgb: 0x999999)
        return switchBtn
    }()
