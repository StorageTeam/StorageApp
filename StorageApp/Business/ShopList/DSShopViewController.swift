...
class DSShopViewController: DSBaseViewController, CurrentShopDelegate, DSDataCenterDelegate {
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        if DSAccount.isLogin() && self.viewModel.isEmpty() {
            DSShopService.requestList(DSShopViewController.SHOPLIST_REQUEST, delegate: self)
        }
    }
    ...
    func networkDidResponseSuccess(identify: Int, header: DSResponseHeader, response: [String : AnyObject], userInfo: [String : AnyObject]?) {
        if header.code == DSResponseCode.Normal {
            if identify == DSShopViewController.SHOPLIST_REQUEST {
                let items = DSShopService.parseList(response)
                
                switch identify {
                case DSShopViewController.SHOPLIST_REQUEST:
    ...
    override func loadView() {
        ...
        let borderWidth : CGFloat = (DSConst.IS_iPhone4() ? 240 : 286)
        var offset = (DSConst.IS_iPhone4() ? 30 : 50)
        ...
        offset = (DSConst.IS_iPhone4() ? 40 : 55)
        ...
        offset = (DSConst.IS_iPhone4() ? 35 : 50)
        let size = (DSConst.IS_iPhone4() ? CGSizeMake(260, 320) : CGSizeMake(260, 340))
    ...
    lazy var bgImgView: UIImageView = {
        ...
        imgView.image = DSImage.dss_bgImage(CGSizeMake(CGRectGetWidth(UIScreen.mainScreen().bounds)
    ...
    lazy var buildingView: DSBuildingImgView = {
        let view = DSBuildingImgView.init(frame: CGRectZero)
        view.backgroundColor = UIColor.whiteColor()
