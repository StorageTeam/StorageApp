...
class DSBuyingController: DSBaseViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, DSDataCenterDelegate{

    var missionItem: DSMissionItem?
    var finishBlock: (Bool -> Void)?
    
    let reqSuccessIdentify = 2001
    let reqFailIdentify = 2002
    
    convenience init(missionItem: DSMissionItem, finish: (Bool -> Void)?) {
        self.init()
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if (indexPath.section == 0) {
            if let proCell = tableView.dequeueReusableCellWithIdentifier(String(DSBuyingProCell)) as? DSBuyingProCell {
                proCell.proImgView.dss_setImageFromURLString((missionItem?.firstPic)!, cdnWidth: DSBuyingProCell.imgCdnWidth())
                proCell.titleLabel.text = missionItem?.title
                return proCell
            }
            
        } else if (indexPath.section == 1){
            if let countCell = tableView.dequeueReusableCellWithIdentifier(String(DSBuyingCountCell)) as? DSBuyingCountCell {
            ...
            if let confirmCell = tableView.dequeueReusableCellWithIdentifier(String(DSBuyingBtnCell)) as? DSBuyingBtnCell {
    ...
    func tableView(tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if (section == 0) {
            if let header = tableView.dequeueReusableHeaderFooterViewWithIdentifier(String(DSBuyingHeaderView)) as? DSBuyingHeaderView{
    ...
    func reqMissionSuccess() {
        if self.missionItem?.goodsID != nil {
            self.showHUD()
            DSBuyingServe.reqMissionSuccess(reqSuccessIdentify,
    ...
    func reqMissionFail() {
        if self.missionItem?.goodsID != nil {
            self.showHUD()
            DSBuyingServe.reqMissionFail(reqFailIdentify,
                                          goodsID: (self.missionItem?.goodsID)!,
                                          delegate: self)
        }
    }
    ...
    func getInputNum() -> Int {
        ...
        for cell in self.tableView.visibleCells {
            if let countCell = cell as? DSBuyingCountCell {
...
//        alertController.view.tag = tag
    ...
    lazy var tableView: UITableView = {
        ...
        table.registerClass(DSBuyingProCell.self, forCellReuseIdentifier: String(DSBuyingProCell))
        table.registerClass(DSBuyingCountCell.self, forCellReuseIdentifier: String(DSBuyingCountCell))
        table.registerClass(DSBuyingBtnCell.self, forCellReuseIdentifier: String(DSBuyingBtnCell))
        table.registerClass(DSBuyingHeaderView.self, forHeaderFooterViewReuseIdentifier: String(DSBuyingHeaderView))
        return table
    }()
